...
ggplot(coverage,aes(x = type)) +
 ...
 theme(strip.text.x = element_text(size = 12))+ scale_x_discrete(labels=c("1" = "Pretraining", "2" = "Calibration","3" = "Oracle")) + xlab("\nWeight Estimation Procedure") + ylab("Coverage\n") + theme(plot.title = element_text(hjust = 0.5)) +  geom_hline(yintercept = 0.8, col = "black") + ylim(c(0.5,1)) + theme(axis.text=element_text(size=10),axis.title=element_text(size=12)) + theme(legend.position = "none") 
#ggsave("~/Project with Rina/Uncorrected WCP Comparison.pdf",width = 8,height = 4,plot = last_plot())
```
...
ggplot(data,aes(x = group_size)) + 
  ...
  geom_hline(yintercept = 0.8, col = "black") + xlab("\nNo. of Groups(K)") + ylab("Coverage\n") +
...
ggplot(data1,aes(y = coverage,x = no.of.observations,col = type)) + geom_line() + 
  geom_hline(yintercept = 1-alpha, col = "black",size = 1) + xlab("\nNumber of Observations(n)") + ylab("Coverage Lower Bound\n") + theme_bw() + ylim(c(0.75,0.92))+
  scale_colour_manual(values = c('1' = 'red','2' = 'green', '3' = "blue"),name = 'No. of Groups', 
          labels = c(expression(K == 10), expression(K == sqrt(n)),expression(K == frac(n,10))))
...
data$type = as.factor(c(rep(1,nrow(data_1)),rep(2,nrow(data_2)),rep(3,nrow(data_3))))
levels(data$type) = c(expression(K == 10), expression(K == sqrt(n)),expression(K == frac(n,10)))
```
...
ggplot(data,aes(y=value,x = no_of_obs, col = variable)) + geom_line()  +
  ...
  scale_color_manual(name = "Coverage Guarantee", labels = c("Existing","Using GWCP"),
                     values = c("Existing" = "plum", "Ours" = "#008080")) +
  theme(strip.background = element_rect(color="black", fill="#99CCFF", size=2, linetype="solid"))+
  theme(strip.text.x = element_text(size = 10))+
  geom_hline(yintercept = 1-alpha, col = "red",size = 1.1) + xlab("\nSize of Training Data(n)") + ylab("Coverage Lower Bound\n") + theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text=element_text(size=12),axis.title=element_text(size=12)) +
  theme(legend.title = element_text(size=10),legend.text = element_text(size=10))
#ggsave("~/Project with Rina/Lower Bound Comparison.pdf",width = 10,height = 4,plot = last_plot())
```
...
ggplot(data,aes(x = group_size)) + 
  geom_errorbar(aes(ymin = lb, ymax = ub), width = 1, color = "blue") +
  geom_point(aes(y = mean,col = "red")) + ylim(c(0.5,1))+
  facet_wrap(~type) + theme_bw() + 
    theme(strip.background = element_rect(color="black", fill="skyblue", size=2, linetype="solid"))+
  theme(strip.text.x = element_text(size = 12))+
  geom_hline(yintercept = 0.8, col = "black") + xlab("\nNo. of Groups(K)") + ylab("Coverage\n") +
  theme(plot.title = element_text(hjust = 0.5)) + theme(legend.position = "None")+ theme(axis.text=element_text(size=12),axis.title=element_text(size=12)) 
